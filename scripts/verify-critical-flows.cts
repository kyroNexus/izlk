...
const { classifyDocumentPath, detectProjectSectionCode, detectSourceDataSubtype, hasToken, isTransientSystemFile } = require('../src/lib/document-classifier') as typeof import('../src/lib/document-classifier')
...
`)
	...
	assert.equal(classifyDocumentPath('ИГИ.pdf'), 'SOURCE_DATA')
	assert.equal(classifyDocumentPath('ГПЗУ.pdf'), 'SOURCE_DATA')
	assert.equal(classifyDocumentPath('Инженерно-геологические изыскания.pdf'), 'SOURCE_DATA')
	assert.notEqual(classifyDocumentPath('логистика.pdf'), 'SOURCE_DATA')
	assert.equal(hasToken('Отчёт ИГИ.pdf', 'иги'), true)
	assert.equal(hasToken('логистика.pdf', 'иги'), false)
	assert.equal(detectSourceDataSubtype('ИГИ.pdf'), 'IGI')
	assert.equal(detectSourceDataSubtype('ГПЗУ участка.pdf'), 'GPZU')
	assert.equal(detectSourceDataSubtype('Топографическая основа.pdf'), 'GEOBASE')
	assert.equal(detectSourceDataSubtype('Топографическая съёмка.pdf'), 'TOPO')
	assert.equal(detectSourceDataSubtype('Стеснённые условия площадки.docx'), 'CONSTRAINTS')
	assert.equal(detectSourceDataSubtype('логистика.pdf'), null)
