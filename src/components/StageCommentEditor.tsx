...
export default function StageCommentEditor({ contractId, stages, comments, canWrite }: { contractId: string; stages: Stage[]; comments: Record<string, ThreadItem[]>; canWrite: boolean }) {
	...
	return <><div className="mt-1.5 flex items-center" aria-label="Прогресс по этапам договора">
		...
		<div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto">
			{threads[stage.key]?.length ? threads[stage.key].map((item) => <div key={item.id} className="rounded-tight bg-raised px-2 py-1.5"><div className="flex items-baseline justify-between gap-2 text-2xs text-faint"><span className="font-semibold text-muted">{item.authorName ?? 'Система'}</span><span>{formatDateTime(item.createdAt)}</span></div><p className="mt-0.5 whitespace-pre-wrap text-xs leading-4">{item.text}</p></div>) : <p className="text-xs text-faint">Сообщений пока нет.</p>}
		</div>
		{canWrite ? <div className="mt-2 flex-none">{error && <p className="mb-1.5 rounded-tight bg-warn-bg px-2 py-1.5 text-xs text-warn">{error}</p>}<textarea autoFocus value={text} onChange={(event) => setText(event.target.value)} maxLength={1000} rows={3} placeholder="Сообщение к этапу — например, «деньги поступят через неделю»" className="w-full resize-none rounded-tight border border-line bg-surface px-2.5 py-2 text-xs outline-none focus:border-brand" /><button type="button" onClick={() => void save()} disabled={saving || !text.trim()} className="mt-2 h-8 w-full rounded-tight bg-brand px-3 text-xs font-semibold text-white disabled:opacity-50">{saving ? 'Отправка…' : 'Отправить'}</button></div> : <p className="mt-2 flex-none text-2xs text-faint">Только просмотр</p>}
	</div>, document.body)}</>
}
