...
function DetailField({ formId, name, label, defaultValue, type = 'text', editable, placeholder }: { formId: string; name: string; label: string; defaultValue: string; type?: string; editable: boolean; placeholder?: string }) {
	return <label className="block"><span className="mb-1 block text-[9.5px] font-semibold uppercase tracking-wide text-faint">{label}</span><input form={formId} name={name} type={type} defaultValue={defaultValue} disabled={!editable} placeholder={placeholder} className={`${field} w-full`} /></label>
}
...
export default function ProductionScheduleRow({ row, editable, save }: { row: ProductionRow; editable: boolean; save: (formData: FormData) => void }) {
	...
	return <>
		<tr className="border-b border-line-soft align-top hover:bg-raised/45">
			...
			<td className="px-2 py-2"><Link href={`/contracts/${row.id}`} className="font-bold text-brand-ink hover:underline">{row.number}</Link><div className="mt-1 text-[10px] text-faint">{row.contractorName}</div></td>
			<td className="px-1 py-1"><input form={formId} name="locationOverride" defaultValue={row.locationOverride || row.objectAddress || ''} disabled={!editable} className={field} /><div className="mt-1 text-[10px] text-faint">срок: {formatDate(row.deadline)}</div></td>
			<td className="px-2 py-2 text-[11px] font-semibold">{row.nextOperationLabel}</td>
		...
		{open && <tr className="border-b border-line-soft bg-raised/30">
			...
			<td colSpan={6} className="p-3">
				<div className="grid grid-cols-2 gap-x-3 gap-y-2 sm:grid-cols-4 lg:grid-cols-6">
					...
					<label className="col-span-2 block sm:col-span-4 lg:col-span-6"><span className="mb-1 block text-[9.5px] font-semibold uppercase tracking-wide text-faint">Примечание</span><textarea form={formId} name="note" defaultValue={row.note} disabled={!editable} className="h-[58px] w-full resize-none rounded border border-line bg-surface p-2 text-[11px] outline-none focus:border-brand/60 disabled:bg-raised" /></label>
				</div>
				<div className="mt-2 text-[10px] text-faint">{row.transfer ? `Передан в производство: ${formatDate(row.transfer)}` : 'Ещё не передан в производство'}</div>
			</td>
		</tr>}
	</>
}
