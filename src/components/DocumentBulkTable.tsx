...
export default function DocumentBulkTable({ documents, canEdit }: { documents: Row[]; canEdit: boolean }) {
	...
	return <div>
		{selection.selected.length > 0 && <div className="sticky top-[8px] z-20 flex flex-wrap items-center gap-2 border-b border-line bg-surface px-[15px] py-2 shadow-sm"><b className="mr-1 text-[12px]">Выбрано: {selection.selected.length}</b>{canEdit && <><select defaultValue="" onChange={(event) => { if (event.target.value) run('kind', { kind: event.target.value }); event.currentTarget.value = '' }} className="h-8 rounded border border-line bg-surface px-2 text-[11px]"><option value="">Тип…</option>{DOCUMENT_KIND_ORDER.map((kind) => <option key={kind} value={kind}>{DOCUMENT_KIND_LABELS[kind]}</option>)}</select><select defaultValue="" onChange={(event) => { if (event.target.value) run('state', { state: event.target.value }); event.currentTarget.value = '' }} className="h-8 rounded border border-line bg-surface px-2 text-[11px]"><option value="">Состояние…</option>{states.map((state) => <option key={state.value} value={state.value}>{state.label}</option>)}</select><button onClick={() => run('confidential', { isConfidential: true })} className="rounded border border-line px-2 py-1 text-[11px] font-semibold">Конфиденциально</button><button onClick={() => run('confidential', { isConfidential: false })} className="rounded border border-line px-2 py-1 text-[11px] font-semibold">Обычные</button><button onClick={() => run('archive')} className="rounded border border-line px-2 py-1 text-[11px] font-semibold">В архив</button><button onClick={() => run('restore')} className="rounded border border-line px-2 py-1 text-[11px] font-semibold">Восстановить</button></>}<button onClick={() => run('download')} className="rounded border border-line px-2 py-1 text-[11px] font-semibold">Скачать ZIP</button><button onClick={selection.clear} className="ml-auto text-[11px] text-muted hover:text-ink">Очистить</button></div>}
