...
function SmallMetric({ label, value, tone, href }: { label: string; value: number; tone: string; href: string }) {
	return <Link href={href} className={`group rounded-xl border border-line px-3 py-2.5 transition duration-200 hover:-translate-y-0.5 hover:shadow-md ${tone}`}>
		<div className="text-2xs font-bold uppercase tracking-[.09em] opacity-70">{label}</div>
		<div className="mt-1 flex items-end justify-between gap-2"><b className="text-xl leading-none">{value}</b><span className="text-xs opacity-0 transition group-hover:opacity-70">→</span></div>
	</Link>
}
...
function DashboardDisclosure({ title, subtitle, children, defaultOpen = true }: { title: string; subtitle: string; children: ReactNode; defaultOpen?: boolean }) {
	...
	return <section className="dashboard-disclosure self-start overflow-hidden rounded-[20px] border border-line bg-surface shadow-sm">
		<button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-controls={contentId} className="flex w-full items-center justify-between gap-3 px-5 py-4 text-left transition hover:bg-raised/55">
			<span><b className="block text-base">{title}</b><span className="mt-0.5 block text-xs text-faint">{subtitle}</span></span>
			<span aria-hidden="true" className={`grid h-8 w-8 place-items-center rounded-lg border border-line bg-brand-soft text-md font-normal leading-none text-brand-ink transition-transform duration-200 ${open ? 'rotate-0' : 'rotate-90'}`}>{open ? '−' : '+'}</span>
		</button>
...
function StatLine({ stat }: { stat: Stat }) {
	return <span className="flex min-w-0 items-center gap-1.5" title={stat.label}><i className={`h-1.5 w-1.5 flex-none rounded-full ${DOT[stat.tone]}`} /><span className="truncate text-2xs text-muted">{STAT_SHORT_LABEL[stat.key]}</span><b className="ml-auto text-2xs text-ink">{stat.count}</b></span>
}
...
export default function DepartmentBoard({ departments, flows, totalContracts, closedContracts, attentionCount, createdToday, funnel, timeline, departmentTimeline, attentionItems, userName, role }: {
	...
	// `document` exists on the client from the first paint, before hydration finishes —
	// checking `typeof document` directly makes the client's pre-hydration render diverge
	// from the server's (which always sees `undefined`), triggering a hydration mismatch.
	// Gating on state that starts `false` and flips in an effect keeps the first render
	// identical on both sides; the quick-nav portal then appears right after hydration.
	...
	return <>
			...
			<div className="relative px-5 py-6 sm:px-6">
				<div className="flex flex-col justify-between gap-5 xl:flex-row xl:items-center">
					<div className="max-w-[690px]">
						<div className="inline-flex items-center gap-2 rounded-full border border-ok/25 bg-ok-bg px-2.5 py-1 text-2xs font-bold uppercase tracking-[.1em] text-ok"><i className="h-1.5 w-1.5 rounded-full bg-ok shadow-[0_0_8px_rgba(37,180,114,.8)]" /> Рабочий центр ИЗЛК</div>
						<h1 id="dashboard-title" className="mt-3 text-[28px] font-bold tracking-[-.04em] sm:text-[32px]">Добрый день, {userName}</h1>
						<p className="mt-1.5 text-base leading-5 text-muted">{roleInfo.description} Выберите показатель или отдел, чтобы сразу открыть нужную выборку.</p>
						<div className="mt-4 flex flex-wrap gap-2"><Link href={roleInfo.primaryHref} className="brand-gradient inline-flex h-9 items-center rounded-control px-3.5 text-xs font-semibold text-white shadow-sm transition hover:-translate-y-px hover:shadow-md">{roleInfo.primaryLabel} →</Link><Link href={roleInfo.secondaryHref} className="inline-flex h-9 items-center rounded-control border border-line bg-surface/80 px-3.5 text-xs font-semibold text-ink transition hover:-translate-y-px hover:bg-raised">{roleInfo.secondaryLabel}</Link></div>
					</div>
				...
				<div className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-2 2xl:grid-cols-4">
					{departments.map((department) => <button key={department.key} type="button" onClick={() => setSelectedDepartmentKey(department.key)} aria-label={`Открыть сводку: ${department.label} отдел`} aria-pressed={selectedDepartment?.key === department.key} className={`group relative overflow-hidden rounded-[18px] border bg-surface p-4 text-left transition duration-300 hover:-translate-y-1 hover:border-brand/35 hover:shadow-[0_16px_35px_rgba(74,48,148,.13)] focus:outline-none focus:ring-4 focus:ring-brand/15 ${selectedDepartment?.key === department.key ? 'border-brand/45 shadow-[0_10px_26px_rgba(74,48,148,.10)]' : 'border-line'}`}>
						...
						<div className="flex items-start justify-between gap-3"><div><h2 className="text-base font-bold">{department.label}</h2><p className="mt-1 min-h-[30px] text-xs leading-4 text-faint">{department.description}</p></div><span className="rounded-xl border border-brand/15 bg-brand-soft px-2.5 py-1 text-sm font-bold text-brand-ink">{department.percent}%</span></div>
						...
						<div className="mt-3 flex items-center justify-between border-t border-line-soft pt-2.5 text-2xs"><span className="text-faint">Готово {department.ready} из {department.total}</span><span className="font-bold text-brand-ink">Открыть →</span></div>
					</button>)}
				</div>
			</div>
		</section>
		...
		<section className="mt-4 overflow-hidden rounded-[20px] border border-line bg-surface shadow-sm">
			<div className="flex flex-wrap items-end justify-between gap-3 border-b border-line-soft px-5 py-4"><div><h2 className="text-md font-bold">Сквозной поток договоров</h2><p className="mt-0.5 text-xs text-faint">Нажмите на отдел — увидите этапы, договоры, риски и загрузку людей.</p></div><Link href="/contracts" className="text-xs font-semibold text-brand-ink hover:underline">Все договоры →</Link></div>
			<div className="grid gap-2 p-3 md:grid-cols-4">{departments.map((department, index) => <button key={department.key} type="button" onClick={() => setSelectedDepartmentKey(department.key)} className="group relative rounded-[14px] border border-line bg-raised/45 p-3 text-left transition hover:-translate-y-px hover:border-brand/30 hover:bg-brand-soft/35"><span className="text-2xs font-bold uppercase tracking-[.1em] text-faint">0{index + 1}</span><b className="mt-1 block text-sm text-ink">{department.label}</b><span className="mt-1 flex items-center justify-between text-2xs text-muted"><span>В потоке {flows.find((flow) => flow.key === department.key)?.queueCount ?? 0}</span><span className="text-brand-ink transition group-hover:translate-x-0.5">Открыть →</span></span></button>)}</div>
		</section>
		...
		{selectedDepartment && createPortal(<div className="fixed inset-0 z-[100] flex items-end justify-center bg-[#151326]/45 p-3 backdrop-blur-[3px] sm:items-center sm:p-6" role="presentation" onMouseDown={() => setSelectedDepartmentKey(null)}>
			<section role="dialog" aria-modal="true" aria-labelledby="department-panel-title" onMouseDown={(event) => event.stopPropagation()} className="department-panel-scroll animate-[department-panel-in_.24s_cubic-bezier(.2,.8,.2,1)] max-h-[calc(100vh-24px)] w-full max-w-3xl overflow-y-auto rounded-[24px] border border-line bg-surface shadow-[0_28px_90px_rgba(16,12,48,.34)] sm:max-h-[calc(100vh-48px)]">
				<div className="relative overflow-hidden border-b border-line bg-gradient-to-br from-brand-soft via-surface to-surface px-5 pb-5 pt-6 sm:px-7"><div className="absolute inset-x-0 top-0 h-[3px] bg-gradient-to-r from-brand via-[#aa97ff] to-ok" /><div className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-brand/10 blur-2xl" /><div className="relative flex items-start justify-between gap-4"><div><div className="inline-flex items-center gap-1.5 rounded-full border border-brand/20 bg-surface/80 px-2.5 py-1 text-2xs font-bold uppercase tracking-[.12em] text-brand-ink"><i className="h-1.5 w-1.5 rounded-full bg-brand" />Рабочая сводка</div><h2 id="department-panel-title" className="mt-3 text-[25px] font-bold tracking-[-.04em] sm:text-[30px]">{selectedDepartment.label} отдел</h2><p className="mt-1.5 max-w-xl text-base leading-5 text-muted">{selectedDepartment.description}</p></div><button type="button" onClick={() => setSelectedDepartmentKey(null)} className="grid h-9 w-9 flex-none place-items-center rounded-xl border border-line bg-surface/80 text-muted transition hover:rotate-90 hover:border-brand/30 hover:text-brand-ink" aria-label="Закрыть сводку"><Icon icon={X} size={18} /></button></div><div className="relative mt-5 h-2 overflow-hidden rounded-full bg-raised"><div className="h-full rounded-full bg-gradient-to-r from-brand to-[#a291ff] transition-all duration-700" style={{ width: `${selectedDepartment.percent}%` }} /></div><div className="relative mt-2 flex items-center justify-between text-xs text-muted"><span>Готовность текущего потока</span><b className="text-brand-ink">{selectedDepartment.percent}%</b></div></div>
				<div className="p-5 sm:p-7">
					<div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{selectedDepartment.stats.map((stat) => <div key={stat.key} className="rounded-[14px] border border-line bg-raised/55 p-3"><div className="flex items-center gap-1.5 text-2xs text-muted"><i className={`h-1.5 w-1.5 rounded-full ${DOT[stat.tone]}`} />{stat.label}</div><b className="mt-2 block text-2xl leading-none text-ink">{stat.count}</b></div>)}</div>
					{/* Краткое превью: сводка выше + до 5 позиций в фокусе. Остальное — только на самой странице отдела, чтобы не дублировать. */}
					{selectedFlow && <div className="mt-5 rounded-[16px] border border-line bg-surface p-4"><div className="flex items-center justify-between gap-2"><div><div className="text-2xs font-bold uppercase tracking-[.12em] text-faint">Договоры и позиции в фокусе</div><p className="mt-1 text-xs text-muted">Сначала вопросы, которым нужен следующий шаг.</p></div>{selectedFlow.riskCount > 0 && <span className="rounded-full bg-warn-bg px-2 py-1 text-2xs font-bold text-warn">Риски {selectedFlow.riskCount}</span>}</div><div className="mt-3 space-y-1.5">{selectedFlow.contracts.length ? selectedFlow.contracts.slice(0, 5).map((row) => <Link key={row.id} href={row.href} className="group flex items-center gap-2 rounded-xl px-2.5 py-2 transition hover:bg-raised"><i className={`h-2 w-2 flex-none rounded-full ${row.attention ? 'bg-warn' : 'bg-ok'}`} /><span className="min-w-0 flex-1"><b className="block truncate text-xs">{row.number}</b><span className="block truncate text-2xs text-faint">{row.stage} · {row.responsible ?? row.contractorName}</span></span><span className="text-faint transition group-hover:translate-x-0.5 group-hover:text-brand-ink">→</span></Link>) : <p className="rounded-xl bg-raised px-3 py-3 text-xs text-faint">Позиции появятся после передачи в отдел.</p>}</div></div>}
					<Link href={`/departments/${selectedDepartment.key}`} className="brand-gradient mt-5 flex h-11 items-center justify-center rounded-control text-base font-semibold text-white shadow-sm transition hover:-translate-y-px hover:shadow-md">Открыть рабочую зону →</Link>
				</div>
			</section>
		</div>, document.body)}
		...
		<section id="stages" className="mt-4 grid scroll-mt-20 gap-4 2xl:grid-cols-2">
			...
			<DashboardDisclosure title="Распределение по этапам" subtitle="Где сейчас находятся договоры">{stages.length ? <div className="space-y-2.5">{stages.map((stage) => <Link key={stage.key} href={`/contracts?stage=${stage.key}`} className="group flex items-center gap-2"><span className="w-[130px] truncate text-xs text-muted group-hover:text-brand-ink" title={stage.label}>{stage.label}</span><div className="h-2 flex-1 overflow-hidden rounded-full bg-raised"><div className="h-full rounded-full bg-gradient-to-r from-brand to-[#ad9bff] transition-all group-hover:brightness-110" style={{ width: `${Math.max(5, Math.round(stage.count / stageMax * 100))}%` }} /></div><b className="w-5 text-right text-xs">{stage.count}</b></Link>)}</div> : <p className="py-4 text-center text-xs text-faint">Договоров пока нет</p>}</DashboardDisclosure>
		</section>
			...
			<section className="dashboard-disclosure overflow-hidden rounded-[20px] border border-line bg-surface shadow-sm">
				<button type="button" onClick={() => setAttentionOpen((value) => !value)} aria-expanded={attentionOpen} aria-controls={attentionContentId} className="flex w-full items-center justify-between gap-3 px-5 py-4 text-left transition hover:bg-raised/55">
					<span className="flex min-w-0 items-center gap-3">
						...
						<span className="min-w-0"><b id="attention-title" className="block text-base">Фокус дня</b><span className="mt-0.5 block truncate text-xs text-faint">{attentionItems.length === 0 ? 'Открытых вопросов нет' : `Открыто ${plural(attentionItems.length, 'вопрос', 'вопроса', 'вопросов')} — нужен следующий шаг`}</span></span>
					</span>
					<span className="flex items-center gap-2"><span className={`rounded-full px-2 py-1 text-2xs font-bold ${attentionItems.some((item) => item.tone === 'danger') ? 'bg-danger-bg text-danger' : attentionItems.length ? 'bg-warn-bg text-warn' : 'bg-ok-bg text-ok'}`}>{attentionItems.length || 'ОК'}</span><span className={`grid h-8 w-8 place-items-center rounded-lg border border-line text-brand-ink transition-transform duration-200 ${attentionOpen ? 'rotate-180' : ''}`}>⌄</span></span>
				</button>
				<div id={attentionContentId} hidden={!attentionOpen} className="border-t border-line-soft px-3 pb-3 pt-2 animate-[dashboard-content-in_.18s_cubic-bezier(.2,.8,.2,1)]">
					{attentionItems.length ? <><div className="grid gap-1">{attentionItems.slice(0, 6).map((item) => <Link key={item.id} href={item.href} className="group flex w-full min-w-0 items-center gap-3 rounded-xl px-3 py-2.5 transition hover:bg-raised"><i className={`h-2 w-2 flex-none rounded-full ${item.tone === 'off' ? DOT.muted : DOT[item.tone]}`} /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold group-hover:text-brand-ink">{item.title}</span><span className="mt-0.5 block truncate text-xs text-faint">{item.group} · {item.detail}</span></span><span className="text-base text-faint transition group-hover:translate-x-0.5 group-hover:text-brand-ink">→</span></Link>)}</div><Link href="/contracts?tab=active" className="mt-2 inline-flex rounded-lg px-3 py-2 text-xs font-semibold text-brand-ink hover:bg-brand-soft">Открыть все активные договоры →</Link></> : <div className="rounded-xl bg-ok-bg px-4 py-3 text-xs text-ok">Сроки, файлы и площадки не требуют отдельного действия.</div>}
				</div>
			</section>
		</section>

		<section id="daily-load" className="mt-4 scroll-mt-20 overflow-hidden rounded-[20px] border border-line bg-surface shadow-sm">
			<div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4"><div><h2 className="text-base font-bold">Нагрузка отделов по дням</h2><p className="mt-0.5 text-xs text-faint">Текущий день — слева. Наведите или нажмите на столбец: расшифровка всегда остаётся на виду.</p></div><div className="flex flex-wrap gap-1.5">{departments.map((department) => <button key={department.key} type="button" onClick={() => { setHistoryDepartment(department.key); setSelectedDate(null) }} className={`rounded-lg px-2.5 py-1.5 text-xs font-semibold transition ${historyDepartment === department.key ? 'bg-brand text-white shadow-sm' : 'bg-raised text-muted hover:bg-brand-soft hover:text-brand-ink'}`}>{department.label}</button>)}</div></div>
			<div className="border-t border-line-soft px-5 pb-5 pt-4"><div className="mb-3 flex flex-wrap gap-3 text-2xs font-semibold text-muted"><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-brand" />В работе</span><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-warn" />Внимание</span><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-faint" />Пауза</span><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-ok" />Готово</span></div>
				{selectedDay ? <div className="mb-3 animate-[fade-in_.18s_ease-out] rounded-xl border border-brand/15 bg-gradient-to-r from-brand-soft/55 via-surface to-surface p-3"><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><span className="text-xs font-bold text-ink">Нагрузка на {selectedDay.date}</span><span className="rounded-full bg-raised px-2 py-0.5 text-2xs font-semibold text-muted">{departments.find((item) => item.key === historyDepartment)?.label}: всего {selectedDay.value.total}</span></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><SmallMetric label="В работе" value={selectedDay.value.working} tone="bg-surface/85 text-brand-ink" href="/contracts?tab=active" /><SmallMetric label="Внимание" value={selectedDay.value.attention} tone="bg-warn-bg/55 text-warn" href="/contracts?tab=active" /><SmallMetric label="На паузе" value={selectedDay.value.paused} tone="bg-raised" href="/contracts?tab=active" /><SmallMetric label="Готово" value={selectedDay.value.done} tone="bg-ok-bg/55 text-ok" href="/contracts?tab=closed" /></div></div> : <div className="mb-3 rounded-xl border border-line bg-raised/45 px-3 py-3 text-center text-xs text-faint">За выбранный период ещё нет сохранённых снимков нагрузки.</div>}
