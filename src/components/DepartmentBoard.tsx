	...
	return <>
		...
		{selectedDepartment && createPortal(<div className="fixed inset-0 z-[100] flex items-end justify-center bg-[#151326]/45 p-3 backdrop-blur-[3px] sm:items-center sm:p-6" role="presentation" onMouseDown={() => setSelectedDepartmentKey(null)}>
			<section role="dialog" aria-modal="true" aria-labelledby="department-panel-title" onMouseDown={(event) => event.stopPropagation()} className="department-panel-scroll animate-[department-panel-in_.24s_cubic-bezier(.2,.8,.2,1)] max-h-[calc(100vh-24px)] w-full max-w-3xl overflow-y-auto rounded-[24px] border border-line bg-surface shadow-[0_28px_90px_rgba(16,12,48,.34)] sm:max-h-[calc(100vh-48px)]">
				<div className="relative overflow-hidden border-b border-line bg-gradient-to-br from-brand-soft via-surface to-surface px-5 pb-5 pt-6 sm:px-7"><div className="absolute inset-x-0 top-0 h-[3px] bg-gradient-to-r from-brand via-[#aa97ff] to-ok" /><div className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-brand/10 blur-2xl" /><div className="relative flex items-start justify-between gap-4"><div><div className="inline-flex items-center gap-1.5 rounded-full border border-brand/20 bg-surface/80 px-2.5 py-1 text-[9px] font-bold uppercase tracking-[.12em] text-brand-ink"><i className="h-1.5 w-1.5 rounded-full bg-brand" />Рабочая сводка</div><h2 id="department-panel-title" className="mt-3 text-[25px] font-bold tracking-[-.04em] sm:text-[30px]">{selectedDepartment.label} отдел</h2><p className="mt-1.5 max-w-xl text-[13px] leading-5 text-muted">{selectedDepartment.description}</p></div><button type="button" onClick={() => setSelectedDepartmentKey(null)} className="grid h-9 w-9 flex-none place-items-center rounded-xl border border-line bg-surface/80 text-muted transition hover:rotate-90 hover:border-brand/30 hover:text-brand-ink" aria-label="Закрыть сводку"><Icon icon={X} size={18} /></button></div><div className="relative mt-5 h-2 overflow-hidden rounded-full bg-raised"><div className="h-full rounded-full bg-gradient-to-r from-brand to-[#a291ff] transition-all duration-700" style={{ width: `${selectedDepartment.percent}%` }} /></div><div className="relative mt-2 flex items-center justify-between text-[10.5px] text-muted"><span>Готовность текущего потока</span><b className="text-brand-ink">{selectedDepartment.percent}%</b></div></div>
				<div className="p-5 sm:p-7">
					...
					{selectedFlow && <div className="mt-5 rounded-[16px] border border-line bg-surface p-4"><div className="flex items-center justify-between gap-2"><div><div className="text-[10px] font-bold uppercase tracking-[.12em] text-faint">Договоры и позиции в фокусе</div><p className="mt-1 text-[10.5px] text-muted">Сначала вопросы, которым нужен следующий шаг.</p></div>{selectedFlow.riskCount > 0 && <span className="rounded-full bg-warn-bg px-2 py-1 text-[10px] font-bold text-warn">Риски {selectedFlow.riskCount}</span>}</div><div className="mt-3 space-y-1.5">{selectedFlow.contracts.length ? selectedFlow.contracts.slice(0, 5).map((row) => <Link key={row.id} href={row.href} className="group flex items-center gap-2 rounded-xl px-2.5 py-2 transition hover:bg-raised"><i className={`h-2 w-2 flex-none rounded-full ${row.attention ? 'bg-warn' : 'bg-ok'}`} /><span className="min-w-0 flex-1"><b className="block truncate text-[11px]">{row.number}</b><span className="block truncate text-[10px] text-faint">{row.stage} · {row.responsible ?? row.contractorName}</span></span><span className="text-faint transition group-hover:translate-x-0.5 group-hover:text-brand-ink">→</span></Link>) : <p className="rounded-xl bg-raised px-3 py-3 text-[11px] text-faint">Позиции появятся после передачи в отдел.</p>}</div></div>}
					<Link href={`/departments/${selectedDepartment.key}`} className="brand-gradient mt-5 flex h-11 items-center justify-center rounded-[10px] text-[13px] font-semibold text-white shadow-sm transition hover:-translate-y-px hover:shadow-md">Открыть рабочую зону →</Link>
				</div>
			</section>
		</div>, document.body)}
