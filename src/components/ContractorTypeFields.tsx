...
export default function ContractorTypeFields({ defaultType = 'LEGAL', defaults = {} }: { defaultType?: ContractorType; defaults?: Defaults }) {
	...
	return <>
		<Field label="Тип контрагента">
			<div className="inline-flex rounded-[10px] border border-line bg-raised p-[3px]" role="radiogroup" aria-label="Тип контрагента">
				{(['LEGAL', 'INDIVIDUAL'] as const).map((value) => (
					<label key={value} className={`cursor-pointer rounded-[7px] px-3 py-1.5 text-[12.5px] font-semibold transition ${type === value ? 'bg-brand text-white shadow-sm' : 'text-muted hover:text-ink'}`}>
		...
		{type === 'INDIVIDUAL' && <div className="flex flex-col gap-[14px] rounded-[11px] border border-line bg-raised/40 p-[14px]">
			<div className="text-[11.5px] text-muted">Обязательные данные для физ. лица</div>
