			...
			{projectSections.length === 0 && missingProjectSections.length === 0 ? (
				...
				<div className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">
					{projectSections.map((s) => { const sourceDocs = s.documents.filter((document) => document.kind === 'PROJECT_DWG'); const finalDocs = s.documents.filter((document) => document.kind === 'PROJECT_PDF'); return (
						<div key={s.id} className="rounded-control border border-line bg-raised/40 p-3.5">
							<div className="inline-flex items-center rounded-tight bg-brand-soft px-2 py-1 text-xs font-bold text-brand-ink">
								{PROJECT_SECTION_LABEL[s.code] ?? s.code}
							</div>
							<div className="mt-[10px] truncate text-base font-medium">
								{s.responsible?.name ?? 'Ответственный не назначен'}
							</div>
							<div className="tnum mt-[4px] text-xs text-faint">
							...
							<div className="mt-3">
								...
								<div className="mt-1 flex flex-wrap gap-1.5">
									{sourceDocs.map((document) => <a key={document.id} href={`/api/documents/${document.id}`} className="rounded-lg border border-line bg-surface px-2 py-1 text-2xs font-bold text-brand-ink transition hover:border-brand/40 hover:bg-brand-soft">↓ DWG</a>)}
									{sourceDocs.length === 0 && <span className="text-2xs text-faint">Пока не загружены</span>}
									{canEdit && <Link href={`/contracts/${contractId}/upload?project=${s.id}`} className="rounded-lg border border-dashed border-line px-2 py-1 text-2xs font-semibold text-muted hover:border-brand/40 hover:text-brand-ink">+ Добавить</Link>}
								</div>
							</div>
							<div className="mt-3">
								<div className="micro-label">Итоговый файл (PDF)</div>
								<div className="mt-1 flex flex-wrap gap-1.5">
									{finalDocs.map((document) => <a key={document.id} href={`/api/documents/${document.id}`} className="rounded-lg border border-line bg-surface px-2 py-1 text-2xs font-bold text-brand-ink transition hover:border-brand/40 hover:bg-brand-soft">↓ PDF</a>)}
									{finalDocs.length === 0 && <span className="text-2xs text-warn">Ещё не загружен</span>}
								</div>
							</div>
						</div>
					)})}
