			...
			{documentsForRegistry.length === 0 ? (
				...
				<div className="space-y-[9px] p-[11px]">
					...
					<div key={d.id} className="interactive-row flex items-center gap-2 rounded-[10px] px-[8px] py-[4px]">
								...
								{isAdminUser && <form action={deleteDocument}><input type="hidden" name="documentId" value={d.id} /><button className="rounded-[7px] border border-danger/20 bg-danger/5 px-2 py-1.5 text-[10.5px] font-semibold text-danger hover:bg-danger/10">Удалить</button></form>}
								</div>
							))}
