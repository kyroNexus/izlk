...
export default function FilterSelect({ name, defaultValue = '', options, placeholder = 'Все' }: { name: string; defaultValue?: string; options: Option[]; placeholder?: string }) {
	...
	return <div ref={root} className="filter-select relative min-w-[220px]">
		...
		<button type="button" onClick={() => setOpen((current) => !current)} aria-haspopup="listbox" aria-controls={listId} aria-expanded={open} className="flex h-10 w-full items-center justify-between gap-3 rounded-control border border-line bg-surface px-3 text-left text-sm font-medium text-ink shadow-sm transition hover:border-brand/45 hover:bg-brand-soft/30 focus:outline-none focus:ring-[3px] focus:ring-brand/16">
		...
		{open && <div id={listId} role="listbox" className="absolute left-0 top-[calc(100%+6px)] z-40 w-[min(340px,calc(100vw-32px))] min-w-full overflow-hidden rounded-control border border-brand/30 bg-surface p-1.5 shadow-[0_18px_42px_rgba(16,12,44,.32)]">
			<div className="app-scrollbar max-h-64 overflow-y-auto pr-1">{options.map((item) => <button key={item.value || '__all'} type="button" role="option" aria-selected={item.value === value} onClick={() => { setValue(item.value); setOpen(false) }} className={`flex w-full items-center rounded-tight px-2.5 py-2 text-left text-sm leading-5 transition ${item.value === value ? 'bg-brand text-white shadow-sm' : 'text-ink hover:bg-brand-soft hover:text-brand-ink'}`}><span className="block w-full break-words">{item.label}</span></button>)}</div>
		</div>}
	</div>
}
