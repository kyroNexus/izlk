...
export default async function NewSitePage({ searchParams }: { searchParams: { error?: string; contract?: string } }) {
	...
	return <><Topbar crumbs={[{ label: 'Главная', href: '/' }, { label: 'Площадки', href: '/sites' }, { label: 'Новая площадка' }]} userName={name.split(' ')[0]} initials={initials(name)} /><div className="workspace-content px-[26px] py-[22px]"><h1 className="text-[26px] font-bold">Новая площадка</h1><p className="mb-[16px] mt-[5px] text-[13px] text-muted">Выберите договор — шифр, заказчик и адрес будут связаны автоматически.</p><div className="max-w-[680px]"><FormError message={searchParams.error} /><Card className="mt-[14px] p-[22px]"><form action={createSite} className="flex flex-col gap-[15px]"><Field label="Договор" required><select name="contractId" required defaultValue={searchParams.contract ?? ''} className={selectClass}><option value="" disabled>Выберите активный договор</option>{contracts.map((contract) => <option key={contract.id} value={contract.id}>№ {contract.number} · {contract.contractor.name}{contract.cipher ? ` · ${contract.cipher}` : ''}</option>)}</select></Field><Field label="Адрес площадки" required><input name="address" required className={inputClass} placeholder="Адрес объекта" /></Field><div className="flex gap-[10px]"><button className="brand-gradient inline-flex h-[40px] items-center rounded-[10px] px-[18px] text-[13.5px] font-semibold text-white">Создать площадку</button><Link href="/sites" className="inline-flex h-[40px] items-center rounded-[10px] border border-line px-[18px] text-[13.5px] font-semibold">Отмена</Link></div></form></Card></div></div></>
}
