...
const WORKFLOW_STAGES: ContractWorkflowStage[] = ['CONTRACT_PREPARATION', 'AWAITING_CONTRACT_SIGNATURE', 'PR1_DEVELOPMENT', 'AWAITING_PR1_SIGNATURE', 'WAITING_DESIGN', 'DESIGN', 'WAITING_PRODUCTION', 'PRODUCTION', 'AWAITING_SHIPMENT', 'SHIPPED', 'INSTALL_KZH', 'INSTALL_KM', 'CLOSED']
...
function ProjectBadges({ sections }: { sections: Array<{ code: SectionCode; queueStatus: string; documents: Array<{ id: string }> }> }) {
	...
	return <span className="mt-1 flex flex-wrap gap-1">{sections.filter((item) => ['KM', 'KZH', 'AR'].includes(item.code)).map((item) => {
		...
		return <span key={item.code} className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-2xs font-bold ${ready ? 'bg-ok-bg text-ok' : working ? 'bg-warn-bg text-warn' : 'bg-off-bg text-faint'}`}>{SECTION_FILTER_LABEL[item.code]} <Icon icon={ready ? Check : working ? Circle : Minus} size={11} /></span>
	})}</span>
}
...
export default async function ContractsPage({ searchParams }: { searchParams: { q?: string; tab?: string; year?: string; kind?: string; stage?: string; section?: string; department?: string; focus?: string } }) {
	...
	const contracts = await prisma.contract.findMany({
		...
		include: {
			...
			stageCommentLog: { where: { deletedAt: null }, orderBy: { createdAt: 'asc' }, select: { id: true, stage: true, text: true, createdAt: true, author: { select: { name: true } }, attachments: { select: { id: true, fileName: true, sizeBytes: true, isImage: true } } } },
	...
	return <>
		...
		<div className="workspace-content">
			<div className="mb-[18px] flex items-start gap-4">
				<div><h1 className="text-2xl font-bold tracking-[-0.02em]">Договоры</h1><div className="mt-1 text-base text-muted">{plural(visibleContracts.length, 'договор', 'договора', 'договоров')} в выбранном разделе</div></div>
				{canWrite(user) && <div className="ml-auto flex gap-2"><Link href="/contracts/import" className="inline-flex h-control items-center rounded-control px-3 text-sm font-semibold text-muted hover:bg-raised hover:text-ink">Загрузить</Link><Link href="/contracts/new" className="brand-gradient inline-flex h-control items-center rounded-control px-3.5 text-base font-semibold text-white">+ Новый договор</Link></div>}
			</div>
			{(focus || selectedDepartment || selectedStage) && <div className="mb-[14px] flex flex-wrap items-center justify-between gap-3 rounded-xl border border-brand/20 bg-brand-soft/55 px-4 py-3"><div><b className="block text-sm text-brand-ink">Показаны договоры: {focus ? FOCUS_LABEL[focus] : selectedDepartment ? `${DEPARTMENT_LABEL[selectedDepartment]} отдел` : `этап — ${WORKFLOW_STAGE_LABEL[selectedStage!]}`}</b><span className="mt-0.5 block text-xs text-muted">{focus || selectedDepartment ? 'Фильтр применён с главной страницы.' : 'Фильтр по этапу применён.'} Его можно изменить или сбросить ниже.</span></div><Link href="/contracts" className="rounded-lg border border-brand/20 bg-surface px-3 py-1.5 text-xs font-semibold text-brand-ink hover:bg-brand-soft">Сбросить</Link></div>}
			...
			<div className="work-toolbar mb-[15px] flex flex-wrap items-center gap-3 p-2.5">
				<form method="get" className="relative w-[360px] max-w-full">
					...
					<input name="q" defaultValue={q} placeholder="Номер, шифр или контрагент" className="h-control w-full rounded-control border border-line bg-surface pl-9 pr-3 text-base outline-none focus:border-brand focus:ring-2 focus:ring-brand/20" />
				</form>
				<div className="inline-flex gap-1 rounded-control border border-line bg-raised p-1">{TABS.map((item) => <Link key={item.key} href={href({ tab: item.key })} className={`rounded-tight px-3 py-1.5 text-sm font-semibold ${item.key === tabKey ? 'brand-gradient text-white' : 'text-muted hover:text-ink'}`}>{item.label}</Link>)}</div>
				<details className="relative">
					<summary className={`flex h-control cursor-pointer list-none items-center gap-1.5 rounded-control border px-3 text-sm font-semibold ${selectedStage ? 'border-brand/40 bg-brand-soft text-brand-ink' : 'border-line bg-surface text-muted hover:bg-raised'}`}>Фильтры{selectedStage && <i className="h-[6px] w-[6px] rounded-full bg-brand" aria-hidden="true" />}</summary>
					<form method="get" className="absolute right-0 z-20 mt-2 flex min-w-[280px] items-center gap-2 rounded-control border border-line bg-surface p-2 shadow-[var(--shadow-float)]">
					...
					<button className="h-control rounded-control border border-line bg-surface px-3 text-sm font-semibold hover:bg-raised">Применить</button>
					</form>
				</details>
				{(selectedYear || selectedKinds.length || selectedSections.length || selectedStage || q) && <Link href={href({ year: null, kind: null, section: null, keepFolder: false })} className="text-sm font-medium text-brand-ink hover:underline">Сбросить</Link>}
			</div>

			<div className="grid items-start gap-3.5 2xl:grid-cols-[220px_minmax(0,1fr)]">
				<Card className="overflow-hidden shadow-[0_10px_26px_rgba(25,22,45,.045)]">
					<div className="border-b border-line bg-gradient-to-br from-brand-soft/80 to-surface px-4 py-3.5"><div className="flex items-center gap-2 text-base font-bold"><span className="grid h-7 w-7 place-items-center rounded-lg bg-brand text-white"><FolderIcon /></span><span>Архив договоров</span></div><div className="ml-9 mt-0.5 text-xs text-faint">Все договоры по годам</div></div>
					<div className="max-h-[330px] overflow-y-auto p-2 2xl:max-h-[680px]">
						<Link href={href({ year: null, kind: null })} className={`mb-1 flex items-center justify-between rounded-lg px-2.5 py-2 text-sm ${!selectedYear ? 'bg-brand-soft font-bold text-brand-ink' : 'hover:bg-raised'}`}><span>Все годы</span><span className="text-faint">{contracts.length}</span></Link>
						{years.map((year) => <Link key={year} href={href({ year, kind: null })} className={`mb-1 flex items-center gap-2 rounded-lg px-2.5 py-2.5 text-sm font-bold hover:bg-raised ${selectedYear === year ? 'bg-brand-soft text-brand-ink' : ''}`}><span className="text-faint"><FolderIcon /></span><span className="flex-1">{year}</span><span className="rounded-full bg-raised px-2 py-0.5 text-2xs font-semibold text-faint">{countFor(year)}</span></Link>)}
					</div>
				</Card>

				<Card className="overflow-hidden shadow-[0_10px_26px_rgba(25,22,45,.045)]">
					<div className="border-b border-line bg-raised/60 px-4 py-3">
						<div className="flex items-center gap-3"><div><div className="text-base font-bold">{selectedYear ? `${selectedYear} год` : 'Все договоры'}{selectedKinds.length ? ` · ${selectedKinds.map((kind) => KIND_LABELS[kind]).join(', ')}` : ''}{selectedSections.length ? ` · ${selectedSections.map((section) => SECTION_FILTER_LABEL[section]).join(', ')}` : ''}</div><div className="mt-0.5 text-xs text-faint">Можно выбрать несколько типов и разделов одновременно</div></div><span className="ml-auto rounded-full bg-surface px-2.5 py-1 text-xs font-bold text-muted">{visibleContracts.length}</span></div>
					...
					<div className="min-w-[960px]"><div className="grid grid-cols-[170px_72px_minmax(150px,1fr)_minmax(170px,1.2fr)_92px_140px_55px] gap-3 border-b border-line-soft bg-raised px-4 py-2 text-2xs font-bold uppercase tracking-wide text-faint"><span>Номер / шифр</span><span>Тип</span><span>Контрагент</span><span>Объект</span><span>Дата</span><span>Этапы работ</span><span>Файлы</span></div>
						{visibleContracts.map((contract) => {
							...
							const lastComment = stageThread?.[stageThread.length - 1]
