	...
	async function createContractor(formData: FormData) {
		...
		try {
			created = await prisma.contractor.create({
				data: { name: data.name, aliases: parseAliases(data.aliases ?? ''), inn, address: orNull(data.address), phone: orNull(data.phone), email: orNull(data.email) },
				select: { id: true },
			})
		} catch (error) {
		...
		<div className="px-[26px] py-[22px]">
			...
			<div className="max-w-[680px]"><FormError message={searchParams.error} /><Card className="mt-[14px] p-[22px]">
				<form action={createContractor} className="flex flex-col gap-[15px]">
					<Field label="Название организации" required><input name="name" required className={inputClass} placeholder="ООО «Строймонтаж»" /></Field>
