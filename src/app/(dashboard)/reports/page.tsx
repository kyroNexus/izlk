		...
		<>
			...
			<div className="workspace-content">
				<div className="mb-[18px]">
					<div className="flex flex-wrap items-end justify-between gap-3"><h1 className="text-2xl font-bold tracking-[-0.02em]">Отчёты</h1><form className="flex flex-wrap items-end gap-2" method="get"><label className="text-xs text-muted">С <input name="from" type="date" defaultValue={period.from.toISOString().slice(0, 10)} className="ml-1 rounded border border-line bg-surface px-2 py-1 text-ink" /></label><label className="text-xs text-muted">По <input name="to" type="date" defaultValue={period.to.toISOString().slice(0, 10)} className="ml-1 rounded border border-line bg-surface px-2 py-1 text-ink" /></label><button className="h-[30px] rounded border border-line px-3 text-sm font-semibold">Показать</button><a href={`/api/reports/export?from=${period.from.toISOString().slice(0, 10)}&to=${period.to.toISOString().slice(0, 10)}`} className="brand-gradient inline-flex h-[30px] items-center rounded px-3 text-sm font-semibold text-white">Скачать Excel</a></form></div>
				...
				<div className="mb-[18px] grid grid-cols-1 gap-3 md:grid-cols-3">
					<Link href="/projects" className="group relative overflow-hidden rounded-2xl border border-line bg-surface p-4 shadow-[0_1px_2px_rgba(16,24,40,.05)] transition-all duration-200 hover:-translate-y-0.5 hover:border-brand/35 hover:shadow-[0_14px_30px_rgba(88,63,193,.11)]">
						...
						<div className="relative flex h-full flex-col">
							...
							<b className="mt-5 text-md text-ink">Проекты за месяц</b>
							<span className="mt-1 text-sm text-muted">Сводка по разделам КМ, КЖ и АР</span>
							<span className="mt-4 text-sm font-semibold text-brand-ink">Открыть графики <span aria-hidden>→</span></span>
						</div>
					</Link>
					<a href="#commercial-proposals" className="group relative overflow-hidden rounded-2xl border border-line bg-surface p-4 shadow-[0_1px_2px_rgba(16,24,40,.05)] transition-all duration-200 hover:-translate-y-0.5 hover:border-brand/35 hover:shadow-[0_14px_30px_rgba(88,63,193,.11)]">
						...
						<div className="relative flex h-full flex-col">
							...
							<b className="mt-5 text-md text-ink">Отправленные КП</b>
							<span className="mt-1 text-sm text-muted">{commercialProposals.length ? `Ждут ответа: ${proposalCounts.WAITING_RESPONSE} · Ответы: ${responseTotal}` : 'Файлы с типом «Коммерческое предложение»'}</span>
							<span className="mt-4 text-sm font-semibold text-brand-ink">Посмотреть список <span aria-hidden>↓</span></span>
						</div>
					</a>
					<a href="#budget-control" className="group relative overflow-hidden rounded-2xl border border-dashed border-brand/30 bg-brand-soft/35 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-brand/60 hover:bg-brand-soft">
						<div className="relative flex h-full flex-col">
							...
							<b className="mt-5 text-md text-ink">Свой отчёт</b>
							<span className="mt-1 text-sm text-muted">Соберите оперативную сводку из бюджета и дневных отчётов</span>
							<span className="mt-4 text-sm font-semibold text-brand-ink">Открыть план / факт <span aria-hidden>→</span></span>
						</div>
					</a>
				</div>
						...
						<Card>
							...
							<div className="px-4 py-3.5">
								<div className="mb-[8px] flex items-baseline justify-between">
									...
									<span className="tnum text-base font-semibold text-ink">{sectionsPercent}%</span>
								</div>
								<ProgressBar percent={sectionsPercent} tone={sectionsPercent >= 80 ? 'ok' : 'brand'} />
								<Link
									href="/projects"
									className="mt-[12px] inline-block text-sm text-brand-ink hover:underline"
								>
				...
				<div id="commercial-proposals" className="mt-[14px] scroll-mt-6">
					<Card>
						...
						{commercialProposals.length === 0 ? <EmptyState text="КП появятся здесь автоматически после загрузки файла с типом «Коммерческое предложение»." /> : <><div className="grid grid-cols-2 gap-px border-b border-line-soft bg-line-soft sm:grid-cols-5">{(Object.keys(PROPOSAL_STATUS) as CommercialProposalStatus[]).map((status) => <div key={status} className="bg-surface px-4 py-3"><div className="text-xs text-faint">{PROPOSAL_STATUS[status].label}</div><div className="tnum mt-1 text-md font-bold text-ink">{proposalCounts[status]}</div></div>)}</div><div>{commercialProposals.slice(0, 6).map((proposal) => { const status = PROPOSAL_STATUS[proposal.proposalStatus]; return <div key={proposal.id} className="flex items-center gap-3 border-b border-line-soft px-4 py-2.5 last:border-b-0 transition-colors hover:bg-raised"><FileIcon fileName={proposal.fileName} /><Link href={`/documents/${proposal.id}`} className="min-w-0 flex-1"><span className="block truncate text-base font-semibold">{proposal.fileName}</span><span className="mt-[2px] block truncate text-xs text-muted">№ {proposal.contract.number} · {proposal.contract.contractor.name}{proposal.proposalSentAt ? ` · отправлено ${formatDate(proposal.proposalSentAt)}` : ''}</span></Link><Chip tone={status.tone}>{status.label}</Chip></div>})}</div></>}
					</Card>
				</div>
					...
					<div className="mt-[14px] grid grid-cols-1 gap-3.5 xl:grid-cols-2">
						<Card id="budget-control">
							...
							<div className="px-4 py-3.5">
								<div className="grid grid-cols-2 gap-2 text-sm">
									<div className="rounded-tight bg-raised p-2.5 text-muted">План затрат<br/><b className="tnum mt-[3px] block text-base text-ink">{formatMoney(costSummary.plan)}</b></div>
									<div className="rounded-tight bg-raised p-2.5 text-muted">Факт по отчётам<br/><b className="tnum mt-[3px] block text-base text-warn">{formatMoney(costSummary.actual)}</b></div>
								</div>
								{costSummary.plan > 0 ? <>
									<div className="mt-[14px] flex items-center justify-between text-sm"><span className="text-muted">Освоение бюджета</span><span className={`tnum font-semibold ${costSummary.actual > costSummary.plan ? 'text-danger' : 'text-ink'}`}>{costPercent}%</span></div>
									...
									<div className={`mt-[7px] text-sm ${costSummary.actual > costSummary.plan ? 'text-danger' : 'text-muted'}`}>{costSummary.actual > costSummary.plan ? `Перерасход: ${formatMoney(costSummary.actual - costSummary.plan)}` : `Остаток бюджета: ${formatMoney(costSummary.plan - costSummary.actual)}`}</div>
								</> : <div className="mt-[13px] rounded-tight bg-raised px-2.5 py-2 text-sm text-muted">Добавьте СМР, МК и доставку в карточках договоров — здесь появится контроль общего бюджета.</div>}
								<div className="mt-[13px] flex gap-3.5 border-t border-line-soft pt-2.5 text-xs text-muted"><span>КЖ: <b className="tnum text-ink">{formatMoney(kjActual)}</b></span><span>КМ: <b className="tnum text-ink">{formatMoney(kmActual)}</b></span></div>
							</div>
						</Card>
						...
						<Card>
							...
							{budgetRisks.length === 0 ? <EmptyState text="Сначала заполните разбивку бюджета в договоре." /> : <div>{budgetRisks.map((row) => {
								...
								return <Link key={row.id} href={`/contracts/${row.id}`} className="block border-b border-line-soft px-4 py-2.5 last:border-b-0 hover:bg-raised"><div className="flex items-center justify-between gap-2.5"><span className="font-medium text-base">{row.number}</span><span className={`tnum text-sm font-semibold ${percent > 100 ? 'text-danger' : percent >= 80 ? 'text-warn' : 'text-ink'}`}>{percent}%</span></div><div className="mt-[6px]"><ProgressBar percent={percent} tone={percent > 100 ? 'danger' : percent >= 80 ? 'warn' : 'brand'} height={5} /></div><div className="mt-[5px] flex justify-between text-xs text-muted"><span>факт {formatMoney(row.actual, row.currency)}</span><span>план {formatMoney(row.plan, row.currency)}</span></div></Link>
							})}</div>}
						</Card>
					</div>
				)}
