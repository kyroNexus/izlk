...
export default async function TrashPage() {
  ...
  const actions = (id: string, restore: (f: FormData) => Promise<void>, purge: (f: FormData) => Promise<void>) => <div className="flex gap-2"><form action={restore}><input type="hidden" name="id" value={id}/><button className="rounded-lg border border-ok/25 bg-ok-bg px-2.5 py-1.5 text-[11px] font-semibold text-ok">Вернуть</button></form><form action={purge}><input type="hidden" name="id" value={id}/><button className="rounded-lg border border-danger/25 bg-danger/10 px-2.5 py-1.5 text-[11px] font-semibold text-danger">Удалить навсегда</button></form></div>
  return <><Topbar crumbs={[{label:'Главная',href:'/'},{label:'Корзина'}]} userName={(user.name ?? '').split(' ')[0]} initials={initials(user.name ?? user.email ?? 'A')} /><div className="mx-auto max-w-[1120px] px-[26px] py-[22px]"><div className="mb-5"><h1 className="text-[26px] font-bold tracking-[-.02em]">Корзина</h1><p className="mt-1 text-[13px] text-muted">Записи хранятся 30 дней. «Удалить навсегда» убирает их только из системы: физические файлы на сервере не удаляются.</p></div><div className="grid gap-4 lg:grid-cols-2"><Card className="overflow-hidden"><div className="border-b border-line px-4 py-3 text-[14px] font-bold">Договоры · {contracts.length}</div>{contracts.length ? contracts.map(item => <div className="border-b border-line-soft px-4 py-3 last:border-0" key={item.id}><div className="flex items-start justify-between gap-3"><div><b className="text-[12.5px]">№ {item.number}</b><div className="mt-1 text-[11px] text-muted">{item.contractor.name} · до {formatDateTime(expiration(item.deletedAt!))}</div></div>{actions(item.id, restoreContract, purgeContract)}</div></div>) : <EmptyState text="Удалённых договоров нет" />}</Card><Card className="overflow-hidden"><div className="border-b border-line px-4 py-3 text-[14px] font-bold">Файлы · {documents.length}</div>{documents.length ? documents.map(item => <div className="border-b border-line-soft px-4 py-3 last:border-0" key={item.id}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><b className="block truncate text-[12.5px]">{item.fileName}</b><div className="mt-1 text-[11px] text-muted">Договор № {item.contract.number} · до {formatDateTime(expiration(item.deletedAt!))}</div></div>{actions(item.id, restoreDocument, purgeDocument)}</div></div>) : <EmptyState text="Удалённых файлов нет" />}</Card></div></div></>
}
