...
export default async function SettingsPage({ searchParams }: { searchParams: { error?: string; success?: string } }) {
	...
	async function createUser(formData: FormData) {
		...
		if (name.length < 2 || !/^\S+@\S+\.\S+$/.test(email) || password.length < 8 || !['ADMIN','MANAGER','DESIGNER','VIEWER'].includes(role)) fail('Укажите имя, корректный email и пароль не короче 8 символов')
	...
	return <><Topbar crumbs={[{ label: 'Главная', href: '/' }, { label: 'Настройки' }]} userName={name.split(' ')[0]} initials={initials(name)} />
		<div className="px-[26px] py-[22px]"><div className="mb-[18px]"><h1 className="text-[26px] font-bold">Настройки</h1><div className="mt-[4px] text-[13px] text-muted">Пользователи, права доступа и состояние системы</div></div><div className="mb-[12px] max-w-[760px]"><FormError message={searchParams.error} />{searchParams.success && <div className="rounded-[10px] border border-ok-bd bg-ok-bg px-[13px] py-[9px] text-[12.5px] text-ok">{searchParams.success}</div>}</div>
