		...
		<div className="mx-auto max-w-[1380px] px-[26px] py-[22px]">
			<div className="mb-[16px] flex flex-wrap items-start justify-between gap-3">
				<div className="flex min-w-0 items-center gap-3">
					...
					<div className="min-w-0"><h1 className="truncate text-[24px] font-bold tracking-[-0.02em]">{document.fileName}</h1><p className="mt-1 text-[12.5px] text-muted">{formatBytes(document.sizeBytes)} · {DOCUMENT_KIND_LABELS[document.kind]}</p></div>
				</div>
				<div className="flex gap-2"><a href={`/api/documents/${document.id}`} className="inline-flex h-[39px] items-center rounded-[9px] border border-line bg-surface px-4 text-[12.5px] font-semibold hover:bg-raised">Скачать</a><Link href={`/contracts/${document.contract.id}`} className="inline-flex h-[39px] items-center rounded-[9px] border border-line bg-surface px-4 text-[12.5px] font-semibold hover:bg-raised">К договору</Link></div>
			</div>

			<div className="grid gap-[14px] xl:grid-cols-[minmax(0,1fr)_300px]">
				<Card className="min-h-[560px] overflow-hidden">
					<div className="border-b border-line px-[18px] py-[13px] text-[13px] font-semibold">Просмотр документа</div>
					{nativePreview ? <div className="h-[calc(100vh-240px)] min-h-[560px] overflow-auto bg-raised p-4">{ext === '.pdf' ? <div className="mx-auto max-w-[1100px]"><div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-[9px] border border-line bg-surface px-3 py-2 text-[11.5px] text-muted"><span>Страница {pdfPage} из {pdfPageCount}</span><div className="flex items-center gap-3">{pdfPageCount > 1 && <span className="flex items-center gap-2"><Link href={pdfHref(Math.max(1, pdfPage - 1))} aria-disabled={pdfPage === 1} className={`font-semibold ${pdfPage === 1 ? 'pointer-events-none text-faint' : 'text-brand-ink hover:underline'}`}>← Назад</Link><Link href={pdfHref(Math.min(pdfPageCount, pdfPage + 1))} aria-disabled={pdfPage === pdfPageCount} className={`font-semibold ${pdfPage === pdfPageCount ? 'pointer-events-none text-faint' : 'text-brand-ink hover:underline'}`}>Вперёд →</Link></span>}<a href={previewUrl} target="_blank" rel="noreferrer" className="font-semibold text-brand-ink hover:underline">Открыть оригинал ↗</a></div></div><img src={pdfPreviewUrl} alt={`Страница ${pdfPage}: ${document.fileName}`} className="mx-auto h-auto max-w-full rounded-[6px] border border-line bg-white shadow-sm" /></div> : <img src={previewUrl} alt={document.fileName} className="mx-auto h-auto max-h-full max-w-full rounded-[6px] object-contain" />}</div> : hasTextPreview ? <div className="p-[18px]">{archiveEntries.length > 0 ? <div className="max-h-[calc(100vh-290px)] overflow-auto rounded-[9px] border border-line"><div className="border-b border-line bg-raised px-3 py-2 text-[11.5px] text-muted">Содержимое архива · файлов: {archiveEntries.length}</div>{archiveEntries.map((entry) => <div key={entry.name} className="flex items-center justify-between gap-3 border-b border-line-soft px-3 py-2 text-[12px] last:border-0"><span className="min-w-0 break-all">{entry.name}</span><span className="flex-none text-faint">{entry.size == null ? '' : formatBytes(entry.size)}</span></div>)}</div> : sheetRows.length > 0 ? <div className="overflow-auto rounded-[9px] border border-line"><div className="flex flex-wrap items-center gap-2 border-b border-line bg-raised px-3 py-2 text-[11.5px] text-muted"><span>Лист: {sheetName} · строки {(sheetPage - 1) * 100 + 1}–{Math.min(sheetPage * 100, sheetRowsTotal)} из {sheetRowsTotal}</span><span className="ml-auto flex flex-wrap gap-1">{sheetNames.map((name) => <Link key={name} href={sheetHref(name)} className={`rounded px-2 py-1 font-semibold ${name === sheetName ? 'bg-brand text-white' : 'bg-surface text-brand-ink hover:bg-brand-soft'}`}>{name}</Link>)}</span></div><table className="min-w-full text-left text-[12px]"><tbody>{sheetRows.map((row, rowIndex) => <tr key={rowIndex} className="border-b border-line-soft last:border-0">{row.map((cell, cellIndex) => <td key={cellIndex} className={`max-w-[360px] whitespace-pre-wrap px-3 py-2 align-top ${rowIndex === 0 && sheetPage === 1 ? 'bg-raised font-semibold' : ''}`}>{cell || '—'}</td>)}</tr>)}</tbody></table>{sheetPageCount > 1 && <div className="flex items-center justify-between gap-3 border-t border-line px-3 py-2 text-[12px]"><Link href={sheetHref(sheetName, Math.max(1, sheetPage - 1))} aria-disabled={sheetPage === 1} className={sheetPage === 1 ? 'pointer-events-none text-faint' : 'font-semibold text-brand-ink hover:underline'}>← Предыдущие строки</Link><span className="text-muted">Страница {sheetPage} из {sheetPageCount}</span><Link href={sheetHref(sheetName, Math.min(sheetPageCount, sheetPage + 1))} aria-disabled={sheetPage === sheetPageCount} className={sheetPage === sheetPageCount ? 'pointer-events-none text-faint' : 'font-semibold text-brand-ink hover:underline'}>Следующие строки →</Link></div>}</div> : <><pre className="max-h-[calc(100vh-360px)] overflow-auto whitespace-pre-wrap break-words rounded-[9px] bg-raised p-4 font-sans text-[13px] leading-6 text-ink">{textPreview || 'В документе нет извлекаемого текста.'}</pre>{textPageCount > 1 && <div className="mt-3 flex items-center justify-between gap-3 text-[12px]"><Link href={textHref(Math.max(1, textPage - 1))} aria-disabled={textPage === 1} className={textPage === 1 ? 'pointer-events-none text-faint' : 'font-semibold text-brand-ink hover:underline'}>← Назад</Link><span className="text-muted">Часть {textPage} из {textPageCount}</span><Link href={textHref(Math.min(textPageCount, textPage + 1))} aria-disabled={textPage === textPageCount} className={textPage === textPageCount ? 'pointer-events-none text-faint' : 'font-semibold text-brand-ink hover:underline'}>Вперёд →</Link></div>}</>}</div> : <div className="grid min-h-[520px] place-items-center p-8 text-center"><div><FileIcon fileName={document.fileName} /><h2 className="mt-4 text-[17px] font-bold">Предпросмотр недоступен для этого формата</h2><p className="mx-auto mt-2 max-w-[430px] text-[13px] leading-6 text-muted">{previewError || tooLargeForPreview ? 'Файл слишком большой для безопасного просмотра в браузере.' : 'DWG и старый DOC открываются в профильной программе. Сам файл уже лежит в системе и доступен в один клик.'}</p><a href={`/api/documents/${document.id}`} className="brand-gradient mt-5 inline-flex h-[40px] items-center rounded-[9px] px-4 text-[12.5px] font-semibold text-white">Скачать файл</a></div></div>}
				</Card>
				<Card className="h-fit p-[17px]"><div className="text-[14px] font-bold">Сведения</div><dl className="mt-4 grid gap-3 text-[12px]"><div><dt className="text-faint">Договор</dt><dd className="mt-1 font-semibold">№ {document.contract.number}{document.contract.cipher ? ` · ${document.contract.cipher}` : ''}</dd></div><div><dt className="text-faint">Контрагент</dt><dd className="mt-1 font-semibold">{document.contract.contractor.name}</dd></div><div><dt className="text-faint">Раздел</dt><dd className="mt-1">{document.executiveDoc?.name ?? DOCUMENT_KIND_LABELS[document.kind]}</dd></div><div><dt className="text-faint">Добавлен</dt><dd className="mt-1">{formatDate(document.createdAt)}{document.uploadedBy?.name ? ` · ${document.uploadedBy.name}` : ''}</dd></div><div><dt className="text-faint">Версия</dt><dd className="mt-1">{document.state === 'SIGNED' ? 'Подписанная' : document.state === 'ARCHIVE' ? 'Архивная' : 'Актуальный исходник'}</dd></div></dl>{proposalMeta && <div className="mt-5 border-t border-line pt-4"><div className="flex items-center justify-between gap-2"><span className="text-[12px] font-semibold">Статус ответа по КП</span><Chip tone={proposalMeta.tone}>{proposalMeta.label}</Chip></div><p className="mt-2 text-[11.5px] leading-5 text-muted">{proposalMeta.hint}</p>{document.proposalSentAt && <p className="mt-2 text-[11.5px] text-muted">Отправлено: {formatDate(document.proposalSentAt)}</p>}{document.proposalRespondedAt && <p className="mt-1 text-[11.5px] text-muted">Ответ получен: {formatDate(document.proposalRespondedAt)}</p>}{canWrite(user) && <form action={updateProposalStatus} className="mt-3 flex gap-2"><select name="proposalStatus" defaultValue={document.proposalStatus} className={`${selectClass} h-[34px] min-w-0 flex-1 text-[12px]`}><option value="DRAFT">Черновик</option><option value="SENT">Отправлено</option><option value="WAITING_RESPONSE">Ждём ответ</option><option value="ACCEPTED">Принято</option><option value="REJECTED">Отклонено</option></select><button className="h-[34px] rounded-[9px] bg-brand px-3 text-[11.5px] font-semibold text-white transition hover:brightness-110">Сохранить</button></form>}</div>}</Card>
			</div>
		</div>
	</>
}
