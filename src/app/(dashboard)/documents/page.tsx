	...
	return <><Topbar crumbs={[{ label: 'Главная', href: '/' }, { label: 'Документы' }]} userName={name.split(' ')[0]} initials={initials(name)} />
		<div className="workspace-content px-[26px] py-[22px]">
			...
			<div className="documents-responsive-shell">
				<div className="three-col-panel grid grid-cols-[250px_minmax(0,1fr)_250px] gap-[14px]">
				...
				<Card className="min-h-[650px] overflow-hidden"><div className="flex items-center gap-[10px] border-b border-line px-[14px] py-[12px]"><form method="get" className="flex flex-1 gap-[8px]">{contractId && <input type="hidden" name="contract" value={contractId} />}{kind && <input type="hidden" name="kind" value={kind} />}<input name="q" defaultValue={q} placeholder="Поиск по документам" className={`${inputClass} max-w-[360px]`} /><button className="rounded-[9px] border border-line px-[12px] font-semibold hover:bg-raised">Найти</button></form><span className="text-[11.5px] text-muted">{documents.length} из {total}</span></div>{documents.length === 0 ? <EmptyState text="Документы не найдены" /> : <div><DocumentBulkTable key={`${q}:${kind}:${contractId}:${state}:${page}`} canEdit={canWrite(user)} documents={documents.map((document) => ({ ...document, sizeBytes: Number(document.sizeBytes) }))} /><div className="flex items-center justify-between gap-3 border-t border-line px-[15px] py-[12px] text-[12px]"><span className="text-muted">Страница {page} из {Math.max(1, Math.ceil(total / PAGE_SIZE))}</span><div className="flex gap-2">{page > 1 && <Link href={filterUrl(state, page - 1)} className="rounded-[8px] border border-line px-3 py-1.5 font-semibold hover:bg-raised">← Назад</Link>}{page * PAGE_SIZE < total && <Link href={filterUrl(state, page + 1)} className="rounded-[8px] border border-line px-3 py-1.5 font-semibold hover:bg-raised">Вперёд →</Link>}</div></div></div>}</Card>
