...
function Accent({ group }: { group: Exclude<ActivityFilter, 'all'> }) {
  ...
  return <span className={`grid h-8 w-8 flex-none place-items-center rounded-xl text-2xs font-bold ${className}`}>{label}</span>
}
  ...
  return <>
    ...
    <div className="mx-auto max-w-[1120px] px-[26px] py-[22px]">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div><h1 className="text-2xl font-bold tracking-[-.02em]">Последняя активность</h1><p className="mt-1 text-base text-muted">Кто и что менял: файлы, договоры, этапы и рабочие отчёты.</p></div>
        <span className="rounded-full border border-line bg-surface px-3 py-1.5 text-xs font-semibold text-muted">{countLabel}</span>
      </div>
