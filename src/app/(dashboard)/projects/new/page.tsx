...
export default async function NewProjectPage({ searchParams }: { searchParams: { error?: string } }) {
	...
	return <><Topbar crumbs={[{ label: 'Главная', href: '/' }, { label: 'Проекты', href: '/projects' }, { label: 'Добавить в очередь' }]} userName={name.split(' ')[0]} initials={initials(name)} /><div className="workspace-content"><h1 className="text-2xl font-bold">Добавить проект в очередь</h1><p className="mb-[16px] mt-[5px] text-base text-muted">Договор появится в очереди выбранного конструктора.</p><div className="max-w-[720px]"><FormError message={searchParams.error} /><Card className="mt-[14px] p-[22px]"><form action={createProject} className="flex flex-col gap-3.5"><Field label="Договор" required><select name="contractId" required defaultValue="" className={selectClass}><option value="" disabled>Выберите договор</option>{contracts.map((contract) => <option key={contract.id} value={contract.id}>№ {contract.number} · {contract.contractor.name}{contract.cipher ? ` · ${contract.cipher}` : ''}</option>)}</select></Field><div className="grid grid-cols-1 gap-3.5 md:grid-cols-2"><Field label="Направление" required><select name="code" defaultValue="KM" className={selectClass}><option value="KM">КМ</option><option value="KZH">КЖ</option><option value="AR">АР</option></select></Field><Field label="Конструктор" required><select name="responsibleId" required defaultValue="" className={selectClass}><option value="" disabled>Выберите исполнителя</option>{designers.map((designer) => <option key={designer.id} value={designer.id}>{designer.name}</option>)}</select></Field><Field label="Дней проектирования" required><input type="number" name="durationDays" min="1" max="120" defaultValue="5" className={inputClass} /></Field><Field label="Дедлайн"><input type="date" name="deadline" className={inputClass} /></Field></div><Field label="Комментарий"><textarea name="comment" className={textareaClass} placeholder="Особенности проекта, ограничения, договорённости" /></Field><div className="flex gap-2.5"><button className="brand-gradient inline-flex h-control items-center rounded-control px-4 text-base font-semibold text-white">Добавить в очередь</button><Link href="/projects" className="inline-flex h-control items-center rounded-control border border-line px-4 text-base font-semibold">Отмена</Link></div></form></Card></div></div></>
}
