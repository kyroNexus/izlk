...
export default async function ProjectsPage({ searchParams }: { searchParams: PageParams }) {
	...
	return <>
		...
		<div className="workspace-content">
			<div className="work-hero mb-[16px] flex flex-wrap items-end justify-between gap-3 px-5 py-4">
				<div>
					<h1 className="text-2xl font-bold">Очередь проектирования</h1>
					<p className="mt-[5px] text-base text-muted">Прогноз по рабочим дням, исполнителям и дедлайнам</p>
				</div>
				{canWrite(user) && <Link href="/projects/new" className="brand-gradient inline-flex h-control items-center rounded-control px-4 text-base font-semibold text-white">+ Добавить проект</Link>}
			</div>
...
function ProjectFlowOverview({ sections, readyForProduction, blocked, inProduction }: {
	...
	return <Card className="project-flow-overview mb-[14px] overflow-hidden border-brand/15 bg-gradient-to-br from-brand-soft/55 via-surface to-surface shadow-[0_14px_34px_rgba(25,22,45,.055)]">
		<div className="flex flex-wrap items-center justify-between gap-3 border-b border-line-soft px-4 py-3">
			<div><h2 className="text-base font-bold">Карта потока работ</h2><p className="mt-0.5 text-sm text-muted">Нажмите на раздел — откроется его живая очередь</p></div>
			<Link href="/production-schedule" className="rounded-full border border-brand/25 bg-surface px-3 py-1.5 text-xs font-semibold text-brand-ink transition hover:-translate-y-px hover:border-brand/45">График производства →</Link>
		</div>
		<div className="grid gap-3 p-3.5 lg:grid-cols-[minmax(0,1fr)_minmax(250px,.8fr)]">
			<div className="grid gap-2 sm:grid-cols-3">{sections.map((item) => {
				...
				return <Link key={item.code} href={`/projects?section=${item.code}`} className="project-flow-lane group rounded-[12px] border border-line bg-surface/90 p-3 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-brand/35 hover:shadow-md">
					<div className="flex items-center justify-between gap-2"><b className="text-base">{label}</b><span className="rounded-full bg-brand-soft px-2 py-0.5 text-xs font-bold text-brand-ink">{completed}%</span></div>
					...
					<div className="mt-2 grid grid-cols-3 gap-1 text-xs"><span className="text-brand">● {item.working} раб.</span><span className="text-warn">● {item.paused} пауза</span><span className="text-ok">● {item.ready} готово</span></div>
				</Link>
			})}</div>
			<div className="project-production-bridge rounded-[12px] border border-line bg-surface/90 p-3"><div className="flex items-center justify-between"><b className="text-base">КМ → цех</b><span className="text-xs text-muted">передача</span></div><div className="mt-2 grid grid-cols-3 gap-2 text-center"><Link href="/production-schedule" className="rounded-tight bg-ok-bg px-2 py-2 transition hover:-translate-y-px"><b className="block text-md text-ok">{readyForProduction}</b><span className="text-2xs text-muted">готово</span></Link><Link href="/production-schedule" className="rounded-tight bg-warn-bg px-2 py-2 transition hover:-translate-y-px"><b className="block text-md text-warn">{blocked}</b><span className="text-2xs text-muted">нужен PDF</span></Link><Link href="/production-schedule" className="rounded-tight bg-brand-soft px-2 py-2 transition hover:-translate-y-px"><b className="block text-md text-brand">{inProduction}</b><span className="text-2xs text-muted">в цехе</span></Link></div></div>
		</div>
	</Card>
}
...
function DesignQueue({ section, projects, designers, byDesigner, forecasts, done, atRisk, late, query, userRole, canWrite: editable, action }: {
	...
	return <>
		<div className="mb-[14px] flex flex-wrap gap-1.5">
			{(['KM', 'KZH', 'AR'] as const).map((item) => <Link key={item} href={`/projects?section=${item}`} className={`rounded-full px-4 py-2 text-base font-semibold transition ${section === item ? 'brand-gradient text-white shadow-sm' : 'border border-line bg-surface hover:bg-raised'}`}>{item === 'KZH' ? 'КЖ' : item} проекты</Link>)}
		</div>
		<form className="work-toolbar mb-[14px] flex flex-wrap items-center gap-2 p-2" method="get">
			<input type="hidden" name="section" value={section} />
			<label className="min-w-[220px] flex-1"><span className="sr-only">Найти договор в очереди</span><input name="q" defaultValue={query} placeholder="Найти договор, шифр или контрагента" className="h-10 w-full rounded-control border border-line bg-surface px-3 text-sm outline-none transition focus:border-brand/55" /></label>
			<button className="h-10 rounded-control bg-brand px-4 text-sm font-semibold text-white transition hover:bg-brand-deep">Найти</button>
			{query && <Link href={`/projects?section=${section}`} className="inline-flex h-10 items-center rounded-control px-3 text-sm font-semibold text-muted transition hover:bg-raised hover:text-ink">Сбросить</Link>}
		</form>
		<div className="mb-[14px] grid grid-cols-2 gap-2.5 lg:grid-cols-4">
		...
		{projects.length === 0 ? <Card><EmptyState text={`Очередь ${section === 'KZH' ? 'КЖ' : section} пока пуста`} /></Card> : <div className="grid items-start gap-3.5 xl:grid-cols-[minmax(0,1fr)_330px] xl:gap-4"><div className="grid gap-3.5">{[...byDesigner.entries()].map(([designer, queue]) => <Card key={designer} className="project-queue-card overflow-hidden"><CardHeader title={designer} extra={`${queue.length} проектов`} className="project-queue-card-header" /><div className="flex flex-col">{queue.map((project, index) => {
			...
			return <form action={action} key={project.id} className="border-b border-line-soft transition-colors last:border-b-0 hover:bg-raised/60">
				...
				<details open={index === 0} className="group project-queue-item">
					<summary className="flex cursor-pointer list-none items-center justify-between gap-2.5 p-3.5 marker:content-none"><div className="min-w-0"><span className="font-bold text-brand-ink">№ {project.contract.number}</span><div className="mt-[2px] truncate text-xs text-muted">{project.contract.contractor.name}{project.contract.cipher ? ` · ${project.contract.cipher}` : ''}</div></div><div className="flex flex-none items-center gap-2"><Chip tone={tone}>{finished ? 'Готов' : forecast.gap == null ? 'Без дедлайна' : forecast.gap < 0 ? `Просрочка ${Math.abs(forecast.gap)} дн.` : `Запас ${forecast.gap} дн.`}</Chip><span className="grid h-7 w-7 place-items-center rounded-lg border border-line text-brand transition duration-200 group-open:rotate-180 group-hover:border-brand/35 group-hover:bg-brand-soft">⌄</span></div></summary>
					<div className="border-t border-line-soft px-3.5 pb-3.5"><div className="mt-[10px] flex items-center justify-between gap-3"><Link href={`/contracts/${project.contract.id}`} className="text-xs font-semibold text-brand-ink hover:underline">Открыть договор →</Link><span className="text-xs text-faint">{section === 'KZH' ? 'КЖ' : section} · позиция {index + 1}</span></div><div className="mt-[10px] grid grid-cols-3 gap-1.5 rounded-tight bg-raised p-2 text-xs"><div><span className="text-faint">Начало</span><br/><b>{formatDate(forecast.start)}</b></div><div><span className="text-faint">Готово</span><br/><b>{formatDate(forecast.finish)}</b></div><div><span className="text-faint">Дедлайн</span><br/><b>{formatDate(project.deadline)}</b></div></div>
					{editable && <div className="mt-[10px] grid grid-cols-2 gap-1.5 md:grid-cols-4"><select name="responsibleId" defaultValue={project.responsibleId ?? ''} className="h-[34px] rounded-tight border border-line bg-surface px-2 text-xs">{designers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input name="durationDays" type="number" min="1" max="120" defaultValue={project.durationDays} className="h-[34px] rounded-tight border border-line bg-surface px-2 text-xs" title="Рабочих дней" /><input name="deadline" type="date" defaultValue={project.deadline?.toISOString().slice(0, 10) ?? ''} className="h-[34px] rounded-tight border border-line bg-surface px-2 text-xs" /><select name="status" defaultValue={project.queueStatus} className="h-[34px] rounded-tight border border-line bg-surface px-2 text-xs">{Object.entries(STATUS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select><input name="comment" defaultValue={project.comment ?? ''} placeholder="Комментарий" className="col-span-2 h-[34px] rounded-tight border border-line bg-surface px-2 text-xs md:col-span-3" /><button name="op" value="save" className="h-[34px] rounded-tight bg-brand px-2.5 text-xs font-semibold text-white">Сохранить</button><div className="col-span-2 flex gap-1.5 md:col-span-4"><button name="op" value="up" disabled={index === 0} className="rounded-tight border border-line px-2 py-1 text-xs disabled:opacity-30">↑ Выше</button><button name="op" value="down" disabled={index === queue.length - 1} className="rounded-tight border border-line px-2 py-1 text-xs disabled:opacity-30">↓ Ниже</button></div></div>}
					{userRole === 'DESIGNER' && <div className="mt-[10px] grid gap-1.5"><Link href={`/contracts/${project.contract.id}/upload?project=${project.id}`} className="w-fit rounded-tight border border-brand/30 bg-brand/10 px-2.5 py-1.5 text-xs font-semibold text-brand-ink">Загрузить DWG/PDF</Link><div className="grid grid-cols-[150px_1fr_auto] gap-1.5"><select name="status" defaultValue={project.queueStatus === 'QUEUED' ? 'IN_PROGRESS' : project.queueStatus} className="h-[34px] rounded-tight border border-line bg-surface px-2 text-xs"><option value="IN_PROGRESS">В работе</option><option value="PAUSED">Пауза</option><option value="DONE">Готово</option></select><input name="comment" defaultValue={project.comment ?? ''} placeholder="Комментарий" className="h-[34px] rounded-tight border border-line bg-surface px-2 text-xs" /><button className="h-[34px] rounded-tight bg-brand px-2.5 text-xs font-semibold text-white">Сохранить</button></div></div>}</div>
				</details>
			</form>
...
function ProjectQueueSummary({ section, projects, forecasts, done, atRisk, late }: {
	...
	return <aside className="space-y-[14px] xl:sticky xl:top-[84px]">
		<Card className="overflow-hidden"><CardHeader title={`Сводка ${section === 'KZH' ? 'КЖ' : section}`} extra={`${projects.length} в очереди`} /><div className="grid grid-cols-2 gap-px bg-line-soft"><div className="bg-surface p-4"><span className="text-xs text-faint">Активно</span><b className="mt-1 block text-2xl text-brand">{active.length}</b></div><div className="bg-surface p-4"><span className="text-xs text-faint">Готово</span><b className="mt-1 block text-2xl text-ok">{done}</b></div><div className="bg-surface p-4"><span className="text-xs text-faint">Нужен шаг</span><b className="mt-1 block text-2xl text-warn">{atRisk + late}</b></div><div className="bg-surface p-4"><span className="text-xs text-faint">Всего задач</span><b className="mt-1 block text-2xl text-ink">{projects.length}</b></div></div></Card>
		<Card className="border-brand/20 bg-gradient-to-br from-brand-soft/60 via-surface to-surface p-4"><span className="text-2xs font-bold uppercase tracking-[.12em] text-brand">Следующая позиция</span>{next && nextForecast ? <><Link href={`/contracts/${next.contract.id}`} className="mt-2 block text-md font-bold text-brand-ink hover:underline">№ {next.contract.number}</Link><p className="mt-1 text-sm text-muted">{next.contract.contractor.name}</p><div className="mt-3 rounded-control border border-line bg-surface/85 p-3 text-sm"><span className="text-faint">Прогноз готовности</span><b className="mt-1 block">{formatDate(nextForecast.finish)}</b><span className={`mt-1 block ${nextForecast.gap != null && nextForecast.gap < 0 ? 'text-danger' : 'text-muted'}`}>{nextForecast.gap == null ? 'Дедлайн не указан' : nextForecast.gap < 0 ? `Просрочка ${Math.abs(nextForecast.gap)} дн.` : `Запас ${nextForecast.gap} дн.`}</span></div></> : <p className="mt-2 text-sm text-muted">Все позиции очереди завершены. Можно перейти к следующему разделу.</p>}</Card>
		<Card className="p-4"><h3 className="text-base font-bold">Как работать с очередью</h3><ol className="mt-3 space-y-2 text-sm text-muted"><li><b className="mr-2 text-brand">1.</b>Откройте только нужный договор.</li><li><b className="mr-2 text-brand">2.</b>Назначьте срок и ответственного.</li><li><b className="mr-2 text-brand">3.</b>После PDF переведите раздел в «Готово».</li></ol></Card>
	</aside>
}
