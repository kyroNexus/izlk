...
export default function LoginPage() {
	...
	return (
		<main className="relative min-h-screen overflow-hidden bg-[#f5f4fa] px-4 py-6 dark:bg-[#0c0c14] sm:grid sm:place-items-center">
			...
			<section className="relative mx-auto grid w-full max-w-[940px] overflow-hidden rounded-[26px] border border-white/70 bg-white shadow-[0_28px_80px_rgba(33,24,68,.16)] dark:border-white/10 dark:bg-[#151520] md:grid-cols-[.92fr_1.08fr]">
				<div className="relative flex min-h-[270px] flex-col overflow-hidden bg-[#26145f] p-8 text-white sm:p-10 md:min-h-[610px] md:p-12">
					...
					<div className="relative z-10">
						...
						<div className="mt-2 text-2xs uppercase tracking-[.18em] text-white/45">рабочее пространство</div>
					</div>
					...
					<div className="relative z-10 mt-auto hidden md:block">
						...
						<p className="mt-5 max-w-[355px] text-base leading-6 text-white/65">Контролируйте документы, площадки, задачи и исполнительную документацию без лишних таблиц.</p>
						<div className="mt-8 flex gap-5 text-xs text-white/55"><span>Договоры</span><span>•</span><span>Площадки</span><span>•</span><span>ИД</span></div>
					</div>
				</div>

				<div className="flex min-h-[440px] flex-col justify-center px-7 py-10 sm:px-12 md:px-16">
					<div className="mb-8">
						<div className="text-xs font-bold uppercase tracking-[.16em] text-brand">Корпоративный доступ</div>
						<h2 className="mt-3 text-[29px] font-bold tracking-[-.03em] text-ink">Добро пожаловать</h2>
						<p className="mt-2 text-base text-muted">Войдите с помощью рабочей учётной записи</p>
					</div>

					{error && <div role="alert" aria-live="assertive" className="mb-5 flex items-center gap-2 rounded-control border border-danger-bd bg-danger-bg px-3 py-2.5 text-sm font-medium text-danger"><span className="grid h-5 w-5 place-items-center rounded-full bg-danger text-xs text-white">!</span>{error}</div>}

					<form onSubmit={onSubmit} className="flex flex-col gap-5">
						<label className="flex flex-col gap-2">
							<span className="text-sm font-semibold text-ink">Логин</span>
						...
						<button type="submit" disabled={loading} className="brand-gradient mt-1 flex h-[48px] items-center justify-center rounded-[12px] text-base font-bold text-white shadow-[0_9px_24px_rgba(103,64,226,.25)] transition hover:-translate-y-px hover:shadow-[0_12px_28px_rgba(103,64,226,.32)] disabled:translate-y-0 disabled:cursor-wait disabled:opacity-65">
					...
					<div className="mt-7 flex items-start gap-2 border-t border-line-soft pt-5 text-xs leading-5 text-faint">
			...
			<div className="relative mx-auto mt-4 text-center text-xs text-faint">© 2026 Инновационный завод лёгких конструкций</div>
		</main>
	)
}
