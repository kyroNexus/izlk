...
const PAGE_SIZE = 50
const toDto = (user: SessionUser) => (message: { id: string; text: string; createdAt: Date; authorId: string; author: { id: string; name: string } }) => ({ id: message.id, text: message.text, createdAt: message.createdAt, author: message.author, own: message.authorId === user.id, canDelete: message.authorId === user.id || isAdmin(user) })
