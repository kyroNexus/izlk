...
/**
 * Отдаёт вложение комментария этапа (задача C3) — тот же паттерн, что и у
 * /api/documents/[id] и /api/chats/attachments/[id]: доступ по видимости
 * договора (contractScope), не публичная раздача по одному только id.
 */
async function get(request: Request, { user, requestId }: { user: SessionUser; requestId: string }, { params }: { params: { id: string } }) {
	const attachment = await prisma.stageCommentAttachment.findFirst({
		where: { id: params.id, comment: { deletedAt: null, contract: contractScope(user) } },
	...
	try {
		...
		logger.error('stage_comment_attachment.read_failed', { requestId, route: '/api/stage-comments/attachments/[id]', method: 'GET', userId: user.id, entityType: 'StageCommentAttachment', entityId: attachment.id, error })
